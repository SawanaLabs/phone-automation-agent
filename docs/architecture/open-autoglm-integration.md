...
- Prefer the current BigModel hosted route for backend model calls: `PHONE_AGENT_ENDPOINT=bigmodel` plus `BIGMODEL_TOKEN`. The keyword is resolved in `apps/worker/src/phone_automation_worker/model_endpoints.py`.
- Keep the temporary Android logical display override available for worker-backed demos when screenshots exceed a provider input size limit. Use `adb shell wm size 992x2048` before the run and `adb shell wm size reset` afterward.
- The ModelScope route is now historical and should be configured only through explicit `PHONE_AGENT_BASE_URL`, `PHONE_AGENT_MODEL`, and `PHONE_AGENT_API_KEY` overrides when someone intentionally reproduces that baseline.
...
- `/Users/openclaw/projects/Playground/Open-AutoGLM/HANDOFF-android-modelscope-quickstart-success.md`: historical ModelScope quickstart notes. Use this only when intentionally reproducing that older hosted route outside this monorepo.
