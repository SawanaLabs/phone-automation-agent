...
- Native hosted execution should expose start, continue, confirmation allow, and stop through one hosted runner interface. Pause resume state crosses the React Native bridge as explicit JSON and is parsed into `NativeHostedTaskInput` before entering the native loop.
- Native running stop should reach the Android loop through `stopHostedTask`, not only the React controller's JS stop flag. The native loop should observe a cancellation port and return a `stopped` task snapshot at the next safe checkpoint.
