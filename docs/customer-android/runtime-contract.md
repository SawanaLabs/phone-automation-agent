...
- The API returns exactly one next outcome per step: routine action, pause state, failed state, or finished state.
- The API should maintain session context so later steps can include prior model responses and action history.
