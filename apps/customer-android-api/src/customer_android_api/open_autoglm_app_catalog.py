...
OPEN_AUTOGLM_ANDROID_APP_PACKAGES: dict[str, str] = {
    "微信": "com.tencent.mm",
...
def normalize_open_autoglm_android_app_name(app_name: str) -> str:
    normalized = app_name.strip()
    return OPEN_AUTOGLM_ANDROID_APP_PACKAGES.get(normalized, normalized)


def get_open_autoglm_android_app_name(package_name: str) -> str | None:
    normalized = package_name.strip()
    for app_name, known_package_name in OPEN_AUTOGLM_ANDROID_APP_PACKAGES.items():
        if known_package_name == normalized:
            return app_name
    return None
