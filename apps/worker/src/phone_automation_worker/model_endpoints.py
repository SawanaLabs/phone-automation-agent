...
def resolve_phone_agent_endpoint_from_env(
    *,
    ...
    return ResolvedModelEndpoint(
        ...
        api_key=_resolve_api_key(
            values=values,
            explicit_env_name="PHONE_AGENT_API_KEY",
            preset=preset,
        ),
...
def _required_explicit_value(value: str | None, *, env_name: str) -> str:
    value = _clean(value)
    if value is None:
        raise RuntimeError(f"{env_name} is required for Open-AutoGLM runner mode.")
    return value
...
def _resolve_api_key(
    *,
    ...
    raise RuntimeError(f"{explicit_env_name} is required for Open-AutoGLM runner mode.")
