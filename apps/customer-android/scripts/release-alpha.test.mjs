...
describe("Alpha Sideload APK release", () => {
    ...
    try {
      ...
      const result = await createReleaseArtifacts({
        ...
        runCommand: async (command, args, options) => {
          ...
          if (args[0] === "verify") {
            return {
              stdout: "Signer #1 certificate SHA-256 digest: aa:bb:cc:dd\n",
              stderr: "",
            };
          }
      ...
      await expect(
        readFile(result.releaseNotesPath, "utf8")
      ).resolves.toContain("Customer Phone Agent 0.1.0-alpha.1");
      await expect(
        readFile(result.releaseNotesPath, "utf8")
      ).resolves.toContain("Commit: `abc123`");
      await expect(
        readFile(result.releaseNotesPath, "utf8")
      ).resolves.toContain("Required Android Permissions");
      await expect(
        readFile(result.releaseNotesPath, "utf8")
      ).resolves.toContain("Android version code: `7`");
  ...
  it("discovers apksigner from android/local.properties", async () => {
    ...
    env.ANDROID_HOME = undefined;
    env.ANDROID_SDK_ROOT = undefined;
    env.CUSTOMER_ANDROID_APKSIGNER = undefined;
    env.APKSIGNER = undefined;
