...
const CERTIFICATE_SHA_256_DIGEST_PATTERN =
...
export function extractApkSignerCertificateFingerprint(apksignerOutput) {
  const match = apksignerOutput.match(CERTIFICATE_SHA_256_DIGEST_PATTERN);
...
export function getAndroidVersionCode(env = process.env) {
  ...
  if (!POSITIVE_INTEGER_PATTERN.test(versionCode)) {
    throw new Error(
...
export async function getAlphaReleaseMetadata({
  ...
  if (!POSITIVE_INTEGER_PATTERN.test(alphaNumber)) {
    throw new Error(
      "CUSTOMER_ANDROID_ALPHA_NUMBER must be a positive integer."
    );
  }
...
export async function findApksigner({
  ...
  if (explicitApksigner) {
    ...
    throw new Error(
      `Configured apksigner does not exist: ${resolvedApksigner}`
    );
  }
...
export async function createReleaseArtifacts({
  ...
  const certificateSha256Fingerprint = extractApkSignerCertificateFingerprint(
    apksignerResult.stdout
  );
